        ...
        for url in url_maps:
            if len(url_maps[url]) > 0 and len(all_urls) > 0:
                ...
                while (any(abs(np.asarray(prev_Px).flatten() - np.asarray(Px).flatten()) > 1e-8)):
                    ...
                    Px = Px * P
                print(
                    'Converged in {0} iterations: {1}'.format(i, np.around(np.asarray(Px).flatten().astype(float), 5)))
                self.pr_result = pd.DataFrame(Px, columns=url_matrix.index, index=['score']).T.loc[
                    list(url_maps.keys())]
